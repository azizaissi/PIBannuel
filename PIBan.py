...
current_date_time = cet.localize(datetime(2025, 7, 23, 20, 13))  # Updated to 08:13 PM CET
...
def load_and_preprocess():
    try:
        df = pd.read_csv("VA-2015-2023P.csv", sep=';', encoding='utf-8')
        error_log.append("Fichier chargé comme CSV avec séparateur ';'.")
    except FileNotFoundError:
        st.error("Fichier 'VA-2015-2023P.csv' introuvable. Vérifiez le chemin du fichier.")
        error_log.append("Fichier 'VA-2015-2023P.csv' introuvable.")
        raise
    ...
    return X, y, years, X_df, scaler_X, scaler_y, sectors, macro_rates, events, max(years), y_df, expected_features

try:
    X, y, years, X_df, scaler_X, scaler_y, sectors, macro_rates, events, last_year, y_df, expected_features = load_and_preprocess()
...
ridge_cv = RandomizedSearchCV(ridge, ridge_params, cv=loo, scoring='neg_mean_absolute_error', n_iter=20, random_state=42)
...
elasticnet_cv = RandomizedSearchCV(elasticnet, elasticnet_params, cv=loo, scoring='neg_mean_absolute_error', n_iter=20, random_state=42)
...
huber_cv = RandomizedSearchCV(huber, huber_params, cv=loo, scoring='neg_mean_absolute_error', n_iter=20, random_state=42)
